  ...
  def bubble_sort!(&prc)
    self.length.times do |x|
      self.each_with_index do |el, idx|
        if !self[idx+1].nil? && prc.call(el, self[idx+1]) == 1
...
p sorted = array.bubble_sort! {|num1, num2| num2**2 <=> num1**2}
...
def subwords(word, dictionary)
  ...
  substrings(word).each do |substring|
    output << substring if dictionary.include?(substring)
  end
  output.uniq
end
