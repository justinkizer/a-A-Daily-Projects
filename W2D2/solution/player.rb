class MyInvalidInputClass < StandardError
  attr_reader :msg
  def initialize(msg)
    @msg = msg
  end
...
class HumanPlayer
  ...
  def parse(string)
    if string.downcase.chars.any? { |char| ('a'..'z').include?(char) }
      raise MyInvalidInputClass.new("Invalid input")
    end
